...
const newsItems = [
  {
    ...
    translator: '晚安',
    tags: ['news', 'C#'],
    summary: '上个月很大一部分时间我都在忙大学的事情。下一章节会讲这对发布时间表有什么影响。虽然时间很紧，我们还是在一个大型功能上取得了进展，我觉得很多人会很兴奋。我也会聊一些关于 ETS2LA 未来的重要消息。',
    content: `上个月很大一部分时间我都在忙大学的事情。下一章节会讲这对发布时间表有什么影响。虽然时间很紧，我们还是在一个大型功能上取得了进展，我觉得很多人会很兴奋。我也会聊一些关于 ETS2LA 未来的重要消息。
...
#### 墨染 - SimHub 仪表支持环境可视化显示

视频演示已移至 [ETS2LA 文档站](https://docs.ets2la.cn)

#### Leo (leopri) - Range Rover Interface

<img src="https://tc.ets2la.cn/d/img/25-12/2026-3-16-rr.jpg" width="400" class="rounded-xl border" />
<img src="https://tc.ets2la.cn/d/img/25-12/2026-3-16-rr2.jpg" width="400" class="rounded-xl border" />
*来源不可靠*
...
const renderedContent = computed(() => {
  ...
  if (!item) return ''
  // 使用 marked.parse 配合 gfm 和 breaks 选项，并允许 HTML
  return marked.parse(isExpanded.value ? item.content : item.summary, { async: false }) as string
})
</script>
      ...
      <Card class="bg-card text-card-foreground">
        <CardHeader>
          <CardTitle class="flex items-center justify-center gap-2">
            <span>更新&新闻</span>
            <a
              href="https://docs.ets2la.cn/updates/"
              target="_blank"
              rel="noopener noreferrer"
              class="text-sm font-normal text-muted-foreground hover:text-primary transition-colors flex items-center gap-1"
            >
              查看全部
        ...
        <CardContent class="flex flex-col items-center gap-6">
          <div
            ...
            :key="item.id"
            class="w-full max-w-4xl border rounded-2xl p-6 bg-muted/20 hover:bg-muted/30 transition-colors"
          >
            <!-- 新闻头部 -->
            <div class="flex items-start justify-between gap-4 mb-6">
              <div class="flex-1 text-left">
                ...
                <p class="text-sm text-muted-foreground">
                  作者: {{ item.author }} | 翻译: {{ item.translator }}
                </p>
              </div>
            </div>
            ...
            <div class="text-left">
              <div
                class="prose prose-sm max-w-none dark:prose-invert prose-p:text-muted-foreground prose-headings:text-foreground prose-strong:text-foreground prose-a:text-primary prose-a:no-underline hover:prose-a:underline prose-img:rounded-xl prose-img:border"
              ...
              <button
                ...
                class="mt-6 flex items-center gap-2 text-sm text-primary hover:underline mx-auto px-4 py-2 rounded-full bg-primary/10 hover:bg-primary/20 transition-colors"
              >
