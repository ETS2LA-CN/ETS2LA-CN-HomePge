<template>
  <div :class="cn('group h-72 w-56 [perspective:1000px]', props.class)">
    <div
      :class="
        cn(
          'relative h-full rounded-2xl transition-all duration-1000 [transform-style:preserve-3d]',
          rotation[0],
        )
      "
    >
      ...
      <div
        :class="
          cn(
            'absolute h-full w-full overflow-hidden rounded-2xl border p-4 [backface-visibility:hidden]',
            props.backClass ?? 'bg-black/80 text-slate-200',
            rotation[1],
          )
        "
      >
